...
const useRecentlyViewedProducts = (
  ...
  const { fetchQuery, enableAutoFetch = true, fetchConfig, save } = options;
  ...
  const reset = useAction(removeRecentlyViewedProduct);
  const saveProduct = useAction(saveRecentlyViewedProduct);
